...
class CompanyRepository(BaseRepository):
    """Legacy implementation - methods are being migrated to domain repositories."""
    ...
    async def check_users_limit(self, company_id: str) -> tuple[bool, str]:
        """Check if adding a user would exceed the limit. Returns (allowed, error_message)."""
    ...
    async def check_admins_limit(self, company_id: str) -> tuple[bool, str]:
        """Check if adding an admin would exceed the limit. Returns (allowed, error_message)."""
        limits = await self.get_company_limits(company_id)
    ...
    async def check_documents_limit(self, company_id: str) -> tuple[bool, str]:
        """Check if uploading a document would exceed the limit. Returns (allowed, error_message)."""
        limits = await self.get_company_limits(company_id)
    ...
    async def check_roles_limit(self, company_id: str, admin_id: str) -> tuple[bool, str]:
        """Check if adding a role would exceed the limit. Returns (allowed, error_message)."""
        limits = await self.get_company_limits(company_id)
    ...
    async def update_company_limits(self, company_id: str, max_users: int = None, max_admins: int = None, 
                                   max_documents: int = None, max_roles: int = None) -> dict:
    ...
    async def add_admin(self, company_id: str, admin_id: str, name: str, email: str, modules: Optional[dict] = None):
        ...
        # Check admins limit
        allowed, error_msg = await self.check_admins_limit(company_id)
        if not allowed:
            raise ValueError(error_msg)

        # Get company modules to filter admin modules
        company_modules = await self.get_company_modules(company_id)
            ...
            for k, v in modules.items():
                if k in admin_modules:
                    ...
                    if company_has_module:
                        ...
                        admin_modules[k]["enabled"] = False  # Force disabled if company doesn't have it
        ...
        for k, v in modules.items():
            if k in admin["modules"]:
                ...
                if company_has_module:
                    ...
                    # Force disabled if company doesn't have it
    ...
    async def add_or_update_role(self, company_id: str, admin_id: str, role_name: str, folders: list[str], modules: list, action: str) -> dict:
        ...
        if action == "create" and not existing_role:
            allowed, error_msg = await self.check_roles_limit(company_id, admin_id)
            if not allowed:
                return {
                    "status": "error",
            ...
            if isinstance(modules, list):
                ...
                for module in modules:
                    if isinstance(module, dict):
                        ...
                        if module_name:
                            # Only include modules that company has enabled
    ...
    async def add_user_by_admin(self, company_id: str, added_by_admin_id: str, email: str, company_role: str, assigned_role: str):
        ...
        # Check users limit
        allowed, error_msg = await self.check_users_limit(company_id)
        if not allowed:
            raise ValueError(error_msg)
